...
float regenerate(bool smooth)
{
    ...
    //open_simplex_noise(vase.noise[0].seed, &vase.noise[0].ctx);
...
printf("TIME #-5: %f\n", GetTime());
    ...
    for(size_t x = 0; x < vase.resolution; ++x)
    {
        for(size_t y = 0; y < layersCnt; y++)
        {
            ...
            radius += vase.profileDepth * interpolateSpline(profileCoeff, y*1.0/layersCnt);
            ...
            {
                if (!vase.noise[j].enabled) continue;

                float delta = 1+snoise4
                (
                    0.02*vase.radius*cos(2*PI*xx/(vase.resolution-1)),
                    0.02*vase.radius*sin(2*PI*xx/(vase.resolution-1)),
                    0.02*yy*layerHeightMm,
                    289.0*(vase.noise[j].seed)/1000000000
                );


                radius += delta * vase.noise[j].height;
            }
