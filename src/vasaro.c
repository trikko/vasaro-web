...
void initVase()
{
   ...
   for(size_t i = 0; i<MAX_NOISES; ++i)
   {
      ...
      for(size_t k = 0; k < 10; ++k)
         vase.noise[i].alpha[k] = 1000;

      vase.noise[i].xScale = GetRandomValue(0, 1500);
      vase.noise[i].zScale = GetRandomValue(0, 1000);

      vase.noise[i].twist = GetRandomValue(0, 360);
      vase.noise[i].depth = 5;
      vase.noise[i].seed = GetRandomValue(-1000000000, 1000000000);
      //open_simplex_noise(vase.noise[i].seed, &vase.noise[i].ctx);
   }
}
...
void updateFrame()
{
   ...
   BeginDrawing();
      ...
      for(int i = 0; i < 4; ++i)
      {
         ...
         else DrawTexture(icons[i][1],8,8*(i+1)+40*i,c);
      }
...
void initGui(void)
{
   ...
   // NOISE
   layoutRects[LAY_WINDOW_NOISE] = (Rectangle){ ancNoise.x + 8, ancNoise.y + 0, 360, 445 };
   ...
   layoutRects[LAY_NOISE_STRENGTH_GROUP] = (Rectangle) { ancNoise.x + 194, ancNoise.y + 8 + 24 + 8 + 24 + 8 + 10, 160, 380 - 24 - 8};
   Vector2 ancNoiseAlpha = { layoutRects[LAY_NOISE_STRENGTH_GROUP].x + 20, layoutRects[LAY_NOISE_STRENGTH_GROUP].y };

   for(size_t i = 0; i < 10; ++i)
      layoutRects[LAY_SCROLL_STRENGTH_9 - i] = (Rectangle){ ancNoiseAlpha.x, ancNoiseAlpha.y + 32*i+16, 120, 24 };

   int tmpY = ancNoise.y + 8 + 24 + 8 + 10;
   layoutRects[LAY_CHECKBOX_NOISE_VISIBILE] = (Rectangle){ ancNoise.x + 115, tmpY += 24+8, 16, 16 };
   layoutRects[LAY_TEXT_DEPTH] = (Rectangle){ ancNoise.x + 100, tmpY += 24+8, 80, 24 };
   layoutRects[LAY_TEXT_XSCALE] = (Rectangle){ ancNoise.x + 100, tmpY += 24+8, 80, 24 };
   layoutRects[LAY_TEXT_ZSCALE]= (Rectangle){ ancNoise.x + 100, tmpY += 24+8, 80, 24 };
   layoutRects[LAY_TEXT_TWIST]= (Rectangle){ ancNoise.x + 100, tmpY += 24+8, 80, 24 };
   layoutRects[LAY_BUTTON_SEED]= (Rectangle){ ancNoise.x + 20, tmpY += 24+8, 160, 24 };
}
...
bool noiseXScaleEditMode = false;
bool noiseZScaleEditMode = false;
bool noiseDepthEditMode = false;
bool noiseTwistEditMode = false;
...
bool isEditing()
{
   return vaseProfileDepthEditMode || vaseHeightEditMode || vaseLayerHeightEditMode || vasePointsEditMode || vaseRadiusEditMode ||
      noiseXScaleEditMode || noiseZScaleEditMode || noiseDepthEditMode || noiseTwistEditMode;
}
...
void renderNoiseWindow()
{
   ...
   if (hasNoise)
   {
      ...
      pos += snprintf(&items[pos], 80, "<SELECT>");
   ...
   if (GuiButton(layoutRects[LAY_BUTTON_ADD_NOISE], "ADD")) {
      ...
      vase.noise[noiseCount].depth = 5;
      vase.noise[noiseCount].seed = GetRandomValue(0, INT32_MAX);
      vase.noise[noiseCount].noiseId = globalNoiseId++;
      vase.noise[noiseCount].twist = 100;
      open_simplex_noise(vase.noise[noiseCount].seed, &vase.noise[noiseCount].ctx);
   ...
   if (GuiButton(layoutRects[LAY_BUTTON_REMOVE_NOISE], "REMOVE")) {
      ...
      open_simplex_noise_free(vase.noise[selectedNoise].ctx);
      vase.noise[selectedNoise].ctx = 0;
   ...
   // SINGLE NOISE
   bool isNoiseSelected = noiseCount > 0 && selectedNoise < noiseCount;

   if (noiseCount > 0 && selectedNoise < noiseCount) GuiEnable();
   else GuiDisable();

   GuiGroupBox(layoutRects[LAY_NOISE_STRENGTH_GROUP], "STRENGTH");
   ...
   bool wasVisible = vase.noise[selectedNoise].visible;
   vase.noise[selectedNoise].visible = GuiCheckBox(layoutRects[LAY_CHECKBOX_NOISE_VISIBILE], "ENABLED", vase.noise[selectedNoise].visible);
   if (wasVisible != vase.noise[selectedNoise].visible)
      vase.toUpdate = true;

   if(GuiValueBox(layoutRects[LAY_TEXT_DEPTH], "DEPTH: ", &vase.noise[selectedNoise].depth,0,1000, noiseDepthEditMode)) noiseDepthEditMode = !noiseDepthEditMode;
   if(GuiValueBox(layoutRects[LAY_TEXT_XSCALE], "SCALE X: ", &vase.noise[selectedNoise].xScale,0,5000, noiseXScaleEditMode)) noiseXScaleEditMode = !noiseXScaleEditMode;
   if(GuiValueBox(layoutRects[LAY_TEXT_ZSCALE], "SCALE Z: ", &vase.noise[selectedNoise].zScale,0,2000, noiseZScaleEditMode)) noiseZScaleEditMode = !noiseZScaleEditMode;
   if(GuiValueBox(layoutRects[LAY_TEXT_TWIST], "TWIST: ", &vase.noise[selectedNoise].twist,0,360*2, noiseTwistEditMode)) noiseTwistEditMode =! noiseTwistEditMode;
   if(GuiButton(layoutRects[LAY_BUTTON_SEED], "NEW RANDOM SEED"))
   {
      open_simplex_noise_free(vase.noise[selectedNoise].ctx);
      vase.noise[noiseCount].seed = GetRandomValue(0, INT32_MAX);
      open_simplex_noise(vase.noise[noiseCount].seed, &vase.noise[selectedNoise].ctx);
      vase.toUpdate = true;
   }

   /*
   layoutRects[LAY_CHECKBOX_NOISE_VISIBILE] = (Rectangle){ ancNoise.x + 10, ancNoiseAlpha.y + 8 + 24 + 8 + 24 + 8 + 10, 120, 24 };
   layoutRects[LAY_TEXT_WEIGHT] = (Rectangle){ ancNoise.x + 10, ancNoiseAlpha.y + 8 + 24 + 8 + 24 + 8 + 10, 120, 24 };
   layoutRects[LAY_TEXT_XSCALE] = (Rectangle){ ancNoise.x + 10, ancNoiseAlpha.y + 8 + (24 + 8)*2 + 24 + 8 + 10, 120, 24 };
   layoutRects[LAY_TEXT_YSCALE]= (Rectangle){ ancNoise.x + 10, ancNoiseAlpha.y + 8 + (24 + 8)*3 + 24 + 8 + 10, 120, 24 };
   layoutRects[LAY_TEXT_ZSCALE]= (Rectangle){ ancNoise.x + 10, ancNoiseAlpha.y + 8 +(24 + 8)*4 + 24 + 8 + 10, 120, 24 };
   layoutRects[LAY_TEXT_TWIST
*/
